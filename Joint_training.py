...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_repeats', type=int, default=1, )
    ...
    x_train = torch.tensor(x_train,dtype=torch.float32).to(device).view(len(y_train), -1, num_feature)
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        if args.mode: 
            ...
            R_model.train()
            x_train,  y_train,  x_test,  y_test = BasicDataset().get_data(TRAINING_RATIO)
