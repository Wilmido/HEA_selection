...
def visualization(x, title, filename, mode=False,show=False):
    ...
    plt.legend()
    plt.savefig(f'result/{filename}.pdf', format='pdf',bbox_inches='tight', dpi=700)
    if show:
        plt.show()
...
if __name__ == '__main__':
    ...
    visualization(x_best_tsne,  'best data','best_data_tsne', mode=True)

    num_feature = dataset.get_feature_number()
    model = MyModel(num_feature=num_feature).to(device)
    file_name = r'checkpoint\9_700epochs_5_model.pth'   
    ...
    def get_features(model,x):
        ...
        handle2 = model.c2.register_forward_hook(hook)
    ...
    for i in range(len(features)):
        ...
        visualization(features[i],  title, title+'_tsne')